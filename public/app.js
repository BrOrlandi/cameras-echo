document.addEventListener('DOMContentLoaded', async () => {
  ...
  function applyGlobalMute() {
    ...
    mainVideo.volume = isGlobalMuted ? 0 : 1.0;
    pipVideo.volume = isGlobalMuted ? 0 : 1.0;

    updateSoundIcon();

    if (mainMuteIcon)
      mainMuteIcon.innerHTML = isGlobalMuted ? ICON_MUTED : ICON_UNMUTED;
    if (pipMuteIcon)
      pipMuteIcon.innerHTML = isGlobalMuted ? ICON_MUTED : ICON_UNMUTED;
  }
  ...
  // Check for both cameras offline
  setInterval(() => {
    const now = Date.now();
    const allOffline =
      cameras.length > 0 &&
      cameras.every((c) => c.offlineSince && now - c.offlineSince > 5000);

    if (allOffline) {
      console.log('All cameras offline for > 5s, refreshing page...');
